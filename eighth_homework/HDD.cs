...
namespace eighth_homework
{
	...
	{
		public int SpeedTwoUSB { set; get; } = 480; //480 мб/сек

		public List<int> partitionAmount;

		public HDD(string name, string model)
		{
			Name = name;
			Model = model;
			partitionAmount = new List<int>();			
		}
		public void Add(int partitionMemory)
		{
			partitionAmount.Add(partitionMemory);
			FullMemory = partitionAmount.Sum();
		}
		public override void GetFullInfo()
		{
			base.GetFullInfo();
			WriteLine($"Speed : {SpeedTwoUSB} MB/Sec");			
			WriteLine($"\nActual time for copying with this speed: {TransportingMemory / SpeedTwoUSB} sec");
		}
